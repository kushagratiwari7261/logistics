...
const STEPS = ['Job Type Definition', 'Origin & Destination', 'Port Details & Handling', 'Final Summary'];
const CATEGORIES = [
  'AGENT', 'ARLINE', 'BANK', 'BIKE', 'BIOKER', 'BUYER',
  'CAREER', 'CAREER AGENT'
];

const JOB_TYPE_IMAGES = {
              ...
              <div className="new-shipment-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: '#2d3748', borderBottom: 'none', color: 'white' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button onClick={handleNext} disabled={activeStep >= Math.min(maxStepReached, STEPS.length)} title="Forward" style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(85,95,113,0.4)', border: 'none', cursor: activeStep >= Math.min(maxStepReached, STEPS.length) ? 'not-allowed' : 'pointer', opacity: activeStep >= Math.min(maxStepReached, STEPS.length) ? 0.4 : 1, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}>
                      <ArrowRight size={14} />
                    </button>
                  </div>
                  <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 600, color: 'white' }}>{editingJob ? 'Edit Job' : 'Create Job'}</h1>
                  {editingJob && (
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={() => onMinimize(formConfig.id, { activeStep, maxStepReached, jobType, tradeDirection, formData, editingJob })} title="Minimize" style={{ width: 32, height: 32, background: 'none', border: 'none', borderRadius: '4px', cursor: 'pointer', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}>
                    <Minus size={18} />
                  </button>
                  <button onClick={() => {
                      ...
                      {JOB_TYPES.map((type, index) => (
                        ...
                        >
                          <div className="shipment-card-img-wrap">
                            <img src={JOB_TYPE_IMAGES[type]} alt={type} className="shipment-card-img" />
                          </div>
                          <div className="shipment-card-info">
                            <span className="shipment-type-text">{type}</span>
                            <span className="shipment-type-subtitle">{JOB_TYPE_SUBTITLES[type]}</span>
                          </div>
